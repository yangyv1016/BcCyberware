...
- 修复客户端没有加载义体资源包时出现紫黑错误模型：基础 item_model 改为内置纸张，
  资源包通过专属 CustomModelData 字符串选择器提供义体外观；
- 纸张选择器注入时保留 Oraxen 原有模型分派和顶层设置，重复生成不会无限嵌套；
- Oraxen 改为可选依赖，未安装或未启用时仍可使用完整义体功能，不发送额外资源包；
- 旧部件随档案加载、玩家登录、容器打开和物品实体加载迁移外观，保留实例 UUID、主人与元数据；
- 更新内置示例包、中文配置说明，增加模型选择器合并、缺资源回退和外观迁移回归测试。
